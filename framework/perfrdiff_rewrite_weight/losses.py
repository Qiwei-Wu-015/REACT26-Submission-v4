...
def DiffusionLoss(
    ...
    if encoded_prediction is not None and encoded_prediction.dim() == 4:
        encoded_prediction = encoded_prediction.squeeze(-2)
    if encoded_target is not None and encoded_target.dim() == 4:
    ...
    for loss_name, weight, decoded in zip(losses_type, losses_multipliers, losses_decoded):
        ...
        elif encoded_prediction is not None:
            losses_dict[key] = loss_fn(encoded_prediction, encoded_target, k=k)
        else:
            losses_dict[key] = prediction_emotion.new_tensor(0.0)
