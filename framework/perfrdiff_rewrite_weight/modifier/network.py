...
class ModifierNetwork(nn.Module):
    def __init__(self, input_dim=512, latent_dim=1024, output_dim=None, num_shared_layers=1, lora_rank=0):
        super().__init__()
        ...
        if lora_rank > 0:
            self.branches_A = nn.ModuleList(
                [nn.Linear(latent_dim, int(shape[0]) * lora_rank) for shape in output_dim]
            )
            self.branches_B = nn.ModuleList(
                [nn.Linear(latent_dim, lora_rank * int(shape[1])) for shape in output_dim]
            )
        else:
    ...
    def forward(self, x):
        ...
        if self.lora_rank > 0:
            results = []
            for idx, shape in enumerate(self.output_dim):
                out_dim = int(shape[0])
                in_dim = int(shape[1])
                A = self.branches_A[idx](x).view(out_dim, self.lora_rank)
                B = self.branches_B[idx](x).view(self.lora_rank, in_dim)
                results.append(A @ B)
            return results
        return [
...
class MainNetUnified(nn.Module):
    def __init__(self, cfg, main_net, device):
        ...
        self.lora_rank = cfg.main_model.args.get("lora_rank", 0)
