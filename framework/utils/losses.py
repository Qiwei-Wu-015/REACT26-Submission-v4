...
class DiffusionLoss:
    def __init__(self,
                 ...
                 w_em=2.0,  # emotion
                 w_pred=1.0,  # emotion prediction weight
                 **kwargs):
        ...
        self.w_va = w_va
        self.w_em = w_em
        self.emotion_prediction_loss = EmotionPredictionLoss(
            w_pred=w_pred, w_au=w_au, w_va=w_va, w_em=w_em)
    ...
    def __call__(self, output_decoder):
        ...
        losses_dict["loss_emotion_pred"] = losses_dict["loss"].new_tensor(0.0)
        if "pred_future_emotion" in output_decoder and "target_future_emotion" in output_decoder:
            pred_future = output_decoder["pred_future_emotion"]
            target_future = output_decoder["target_future_emotion"]
            if pred_future is not None and target_future is not None:
                loss_emotion_pred = self.emotion_prediction_loss(pred_future, target_future)
                losses_dict["loss_emotion_pred"] = loss_emotion_pred
                losses_dict["loss"] = losses_dict["loss"] + loss_emotion_pred
...
class EmotionPredictionLoss(nn.Module):
    """Grouped MSE loss for speaker future emotion prediction.
    Splits the 25-d emotion into AU (0:15), VA (15:17), EM (17:25) with configurable weights.
    """
    def __init__(self, w_pred=1.0, w_au=1.0, w_va=5.0, w_em=2.0, **kwargs):
        super(EmotionPredictionLoss, self).__init__()
        self.w_pred = w_pred
        self.w_au = w_au
        self.w_va = w_va
        self.w_em = w_em

    def forward(self, pred_future, target_future):
        loss_au = F.mse_loss(pred_future[:, :, :15], target_future[:, :, :15], reduction='mean')
        loss_va = F.mse_loss(pred_future[:, :, 15:17], target_future[:, :, 15:17], reduction='mean')
        loss_em = F.mse_loss(pred_future[:, :, 17:], target_future[:, :, 17:], reduction='mean')
        loss = (loss_au * self.w_au + loss_va * self.w_va + loss_em * self.w_em) * self.w_pred
        return loss
