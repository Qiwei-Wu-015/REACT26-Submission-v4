...
# 作用：捕获 src 和 dst 之间的乘法交互（二阶交叉项），与 ResampleModalityAdapter 并行
# 零初始化保证训练初期等价于原始模型
# ------------------------------------------------------------------------------------------
class BilinearAdapter(nn.Module):
    ...
    def forward(self, src, dst):
        ...
        if self.src_len != self.dst_len:
            u = u.transpose(1, 2)                    # [B, R, L_src]
            if self.dst_len > self.src_len:
                u = F.interpolate(u, size=self.dst_len, mode='linear', align_corners=True)
            else:
                u = F.adaptive_avg_pool1d(u, self.dst_len)
            u = u.transpose(1, 2)                    # [B, L_dst, R]
...
class StitchBlock(nn.Module):
    def __init__(self, dim, num_heads, modal_lengths, mlp_ratio=4., drop=0.1):
        ...
        # 3.2 First Stitching Layer (Attn Level) — MLP Adapter + Bilinear Adapter
        ...
        for src in range(self.num_modals):
            for dst in range(self.num_modals):
                ...
                self.bilinear_att[f'{src}_{dst}'] = BilinearAdapter(
                    dim, src_len=modal_lengths[src], dst_len=modal_lengths[dst]
                )
        ...
        # 3.4 Second Stitching Layer (FFN Level) — MLP Adapter + Bilinear Adapter
        ...
        for src in range(self.num_modals):
            for dst in range(self.num_modals):
                ...
                self.bilinear_mlp[f'{src}_{dst}'] = BilinearAdapter(
                    dim, src_len=modal_lengths[src], dst_len=modal_lengths[dst]
                )
    ...
    def forward(self, x_list):
        ...
        # --- Stage 2: Cross-Modal Stitching 1 (MLP + Bilinear) ---
        ...
        for src in range(self.num_modals):
            for dst in range(self.num_modals):
                ...
                bilinear_feat = self.bilinear_att[key](current_state[src], current_state[dst])
        ...
        # --- Stage 4: Cross-Modal Stitching 2 (MLP + Bilinear) ---
        ...
        for src in range(self.num_modals):
            for dst in range(self.num_modals):
                ...
                feat = adapter(current_state[src])
                # 双线性分支：捕获 src 和 dst 的乘法交叉项
                bilinear_feat = self.bilinear_mlp[key](current_state[src], current_state[dst])
