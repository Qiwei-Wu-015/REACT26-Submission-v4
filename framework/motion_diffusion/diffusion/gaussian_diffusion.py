...
class FlowMatchingDecoder:
    """
    Rectified Flow / Flow Matching for the Decoder (listener emotion generation).

    Key differences from DDPM:
      - Training:  x_t = (1 - t) * x_0 + t * noise     (linear interpolation)
                   target = x_0                          (model still predicts x_start)
                   loss = MSE(pred_x0, x_0)              (identical loss form)
      - Inference: Euler integration of velocity field v = (x_t - pred_x0) / t
                   10-50 steps instead of 1000, no noise schedule needed.

    The model (TransformerDenoiser) is completely unchanged.
    """

    def __init__(self, cfg, train_timesteps, inference_timesteps):
        self.num_timesteps = train_timesteps
        self.inference_timesteps = inference_timesteps
        self.k = cfg.num_preds
        self.noise_std = cfg.get("noise_std", 1)
        self.rescale_timesteps = cfg.get("rescale_timesteps", False)

    def q_sample(self, x_start, t, noise=None):
        if noise is None:
            noise = th.randn_like(x_start) * self.noise_std
        t_expanded = t.reshape([-1] + [1] * (x_start.ndim - 1))
        return (1.0 - t_expanded) * x_start + t_expanded * noise
    ...
    def ddim_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = th.randn(*shape, device=device) * self.noise_std
        step_size = 1.0 / self.inference_timesteps
        timesteps = th.linspace(1.0, step_size, self.inference_timesteps, device=device)
        if progress:
            from tqdm.auto import tqdm
            timesteps_iter = tqdm(timesteps)
        else:
            timesteps_iter = timesteps
        for i, t_val in enumerate(timesteps_iter):
            t = th.full((shape[0],), t_val, device=device)
            t_next = max(t_val - step_size, step_size)
            with th.no_grad():
                pred_x0 = model.forward_with_cond_scale(
                    img, self._scale_timesteps(t), model_kwargs
                )
                t_exp = t.reshape([-1] + [1] * (img.ndim - 1))
                velocity = (img - pred_x0) / t_exp.clamp_min(1e-5)
                dt = t_val - t_next
                img = img - velocity * dt
                out = {
                    "encoded_prediction": pred_x0,
                    "sample_enc": img,
                }
                yield out
